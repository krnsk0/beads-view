...
The TUI (`src/tui/App.tsx`) is Ink/React running on Bun. Layout is responsive per resize: side-by-side when terminal width > 2 × height, stacked otherwise. Selection is tracked by issue id so refreshes never steal it, and the detail pane's scroll position survives refreshes of the same issue. The legend bar shows the workspace name, dolt mode, a LOCAL/SYNCED badge (from `sync.remote` in `.beads/config.yaml`), and whether live updates are running over SQL or polling.
...
| `q`, `Ctrl-C` | Quit |
| `r` | Manual refresh |
| `a` | Toggle show/hide closed issues |
...
Icons: ⚪ open (ready) · ⏳ in progress · ⛔ blocked (explicit status or unresolved deps) · ✅ closed.
